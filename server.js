...
app.post("/webhook",express.raw({type:"*/*"}),(req,res)=>{try{if(req.get("Kick-Event-Type")!=="chat.message.sent")return res.sendStatus(200);const d=JSON.parse(req.body.toString()),s=d.sender||{},p=s.profile_picture||s.profile_picture_url||"",avatar=typeof p==="string"?p:typeof p?.url==="string"?p.url:"";if(s.username)send({type:"chat",username:s.username,avatar,message:d.content||""});res.sendStatus(200)}catch{res.sendStatus(400)}});
